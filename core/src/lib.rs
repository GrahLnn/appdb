...
pub trait Bridge: Sized + Send {
    /// Persists the bindref value and returns the referenced record id.
    async fn persist_bindref(self) -> anyhow::Result<surrealdb::types::RecordId>;

    /// Rehydrates one bindref value from a stored record id.
    async fn hydrate_bindref(id: surrealdb::types::RecordId) -> anyhow::Result<Self>;
}

/// Recursive runtime seam for `#[bindref]` container shapes.
#[::async_trait::async_trait]
pub trait BindrefShape: Sized + Send {
    /// Stored representation for this bindref shape with `RecordId` leaves.
    type Stored: Clone
        + serde::Serialize
    ...
    /// Persists every bindref leaf and returns the stored recursive shape.
    async fn persist_bindref_shape(self) -> anyhow::Result<Self::Stored>;

    /// Rehydrates a stored recursive shape back into caller-facing values.
    async fn hydrate_bindref_shape(stored: Self::Stored) -> anyhow::Result<Self>;
}
...
{
    async fn persist_bindref(self) -> anyhow::Result<surrealdb::types::RecordId> {
        resolve_store_ref_record_id(self).await
    }
    ...
    T: Bridge + Send,
{
    ...
    async fn persist_bindref_shape(self) -> anyhow::Result<Self::Stored> {
        <Self as Bridge>::persist_bindref(self).await
    }

    async fn hydrate_bindref_shape(stored: Self::Stored) -> anyhow::Result<Self> {
        <Self as Bridge>::hydrate_bindref(stored).await
    }
}

#[::async_trait::async_trait]
impl<T> BindrefShape for Option<T>
where
    T: BindrefShape + Send,
    T::Stored: Send,
{
    ...
    async fn persist_bindref_shape(self) -> anyhow::Result<Self::Stored> {
        match self {
            Some(value) => Ok(Some(
                <T as BindrefShape>::persist_bindref_shape(value).await?,
            )),
            None => Ok(None),
        }
    }
    ...
    async fn hydrate_bindref_shape(stored: Self::Stored) -> anyhow::Result<Self> {
        match stored {
            Some(value) => Ok(Some(
                <T as BindrefShape>::hydrate_bindref_shape(value).await?,
            )),
            None => Ok(None),
        }
    }
    ...
    T::Stored: Send,
{
    ...
    async fn persist_bindref_shape(self) -> anyhow::Result<Self::Stored> {
        ...
        for value in self {
            out.push(<T as BindrefShape>::persist_bindref_shape(value).await?);
        }
        Ok(out)
    }
    ...
    async fn hydrate_bindref_shape(stored: Self::Stored) -> anyhow::Result<Self> {
        ...
        for value in stored {
            out.push(<T as BindrefShape>::hydrate_bindref_shape(value).await?);
        }
        Ok(out)
    }
}
