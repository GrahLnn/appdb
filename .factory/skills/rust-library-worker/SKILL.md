...
6. Run the new or updated tests and confirm they fail for the intended reason before implementation. Record that failing-first proof in the handoff; if the path is structurally impossible, the only honest proof is an already-present passing regression, or a trybuild-style compile-boundary case only becomes meaningful as an initial stderr mismatch after the diagnostic lands, record that exception explicitly and explain why no earlier failing-first step could be shown.
